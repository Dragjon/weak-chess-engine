...
// Main search sorting
// Returns a boolean vector indicating the SEE result for each move
std::vector<bool> sort_moves(Board& board, Movelist& movelist, bool tt_hit, uint16_t tt_move, int32_t ply, SearchInfo search_info) {
    ...
    for (size_t i = 0; i < move_count; i++) {
        ...
        if (tt_hit && move.move() == tt_move) {
            ...
            good_see = see(board, move, 0);
            if (!good_see) score -= 10000000;
